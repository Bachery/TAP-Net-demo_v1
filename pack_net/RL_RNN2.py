...
class PackEngine:
    ...
    def step(self, action, block, draw_img=False):
        ...
        #     pos = None
            ...
            #         save_name='img/p_%s_%.3f' % (self.num_blocks, np.random.rand()) )
            # input('Enter')                    
        ...
        return state, reward, done, pos, C, P, S
    ...
    def get_heightap(self, heightmap_type):
        if heightmap_type == 'normal':
            ...
            print('====> Error in RNN2 engines get map')
...
class PackDataset_rand(Dataset):
    def __init__(self, data_file, blocks_num, data_size):
        ...
        self.gt_blocks = gt_blocks[:12800]
        self.gt_positions = gt_positions[:12800]
        
        # self.data_size = data_size
        self.data_size = 12800

        print(self.gt_blocks.shape)
        print(self.gt_positions.shape)
...
class PackRNN(nn.Module):
    ...
    def __init__(self, block_input_size, block_hidden_size, height_input_size, height_hidden_size, container_width, container_height, heightmap_type, max_blocks_num=10):
    ...
    def forward(self, blocks, blocks_num):
        ...
        # height_map = torch.zeros(batch_size, self.container_width, 1)
        height_map = np.zeros((batch_size, self.container_width, 1)).astype('float32')
        # hit_porb_log = torch.zeros((batch_size, self.max_blocks_num))
        ...
        # if use_cuda:
            # reward = reward.cuda()
            # height_map = height_map.cuda()
            # hit_porb_log = hit_porb_log.cuda()
        ...
        for block_index in range(blocks_num):
            ...
            #     tmp = copy.deepcopy(height_map)
            #     tmp = tmp - np.min(height_map, axis=1)[:,:,None].repeat(self.container_width, 1)
            #     height_vec = self.height_map_conv( torch.from_numpy(tmp).cuda() )
                
            else:
                height_vec = self.height_map_conv( torch.from_numpy(height_map).cuda() )

            # height_vec = self.height_map_conv( height_map.detach() )
            ...
            # hit_porb_log[:, block_index] = torch.log(hit_map.max(1)[0])
            ...
            for batch_index in range(batch_size):
                ...
                _, rw[batch_index], _, pos, C[batch_index], P[batch_index], S[batch_index] = self.engines[batch_index].step( action, block )
        ...
        # for batch_index in range(batch_size):
        #     # positions[batch_index] = engines[batch_index].positions
        #     # reward[batch_index] = engines[batch_index].reward
        #     reward[batch_index] = rw[batch_index]
        ...
        return positions, hit_porb_log, -reward #, C, P, S
...
def calc_positions(net, blocks, container_size, is_train=True):
    ...
    for batch_index in range(batch_size):
        ...
        # tools.draw_container_2d( blocks[batch_index], positions[batch_index], container_size, \
        #     # save_title='C %.3f   S %.3f   P %.3f' % (C[batch_index], S[batch_index], P[batch_index]), \
        #         save_name='./img/RNN2_%d_%d' % ( batch_index, blocks_num))
...
def train(actor, critic, train_size, valid_size, blocks_num, batch_size, epoch_num, learning_rate, save_dir, use_cuda, net_type, is_train=False):
    ...
    for epoch in range(epoch_num):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            #     blks = blks.transpose()
            #     tools.draw_container_2d(blks[:i+1], positions[0], container_size, save_name=save_dir + '/img/pack_%d' % i)
            # input('Stop')
            ...
            if (batch_idx + 1) % log_step == 0:
                ...
                print('    Epoch %d  Batch %d/%d, reward: %2.4f, loss: %2.4f, took: %2.4fs' %
        ...
        with torch.no_grad():
            ...
            for i in range(blocks_num):
                ...
                # tools.draw_container_2d(blks, positions, container_size, save_name=save_dir + '/pack_%d' % i)
            # input('Stop')
            blks = blocks[0].cpu().numpy()
            blks = blks.transpose()
            positions = positions[0]
...
def valid( net, num_samples, blocks_num, save_dir):
    ...
    #     ]
    # ).transpose()
    # blocks = torch.from_numpy(b.astype('float32')).view(1, 2, -1).cuda()
...
if __name__ == '__main__':
    ...
    save_dir = './%s_%s_12800' % (note, heightmap_type)
    # save_dir = note
    print(save_dir)

    container_width, container_height = 5, 50 # container_size
    ...
    if is_train == False:
        checkpoints = save_dir + '/checkpoints/149'
    else:
        checkpoints = None
        
    actor = PackRNN(2, block_hidden_size, container_width, height_hidden_size, container_width, container_height, heightmap_type, blocks_num)
    ...
    if checkpoints is not None:
        ...
        actor.load_state_dict(torch.load('../pack/10/2d-bot-C+P+S-RNN-soft-width-5-diff-pre_eval-note-sh-R-RNN2-2020-05-12-14-1/checkpoints/4/actor_pnet.pt'))
